...
## Wireframes
### Regular Gameplay
![](https://github.com/josephsurrey/Llama-Game/blob/main/Wireframes/gameplay_wireframe.png)
### Game over
![](https://github.com/josephsurrey/Llama-Game/blob/main/Wireframes/gameover_standard_wireframe.png)
...
[constants.py - Llama Game Decomposition](https://github.com/josephsurrey/Llama-Game/blob/main/constants.py%20-%20Llama%20Game%20Decomposition.md)
[Game Class - Llama Game Decomposition](https://github.com/josephsurrey/Llama-Game/blob/main/Game%20Class%20-%20Llama%20Game%20Decomposition.md)
[Llama Class - Llama Game Decomposition](https://github.com/josephsurrey/Llama-Game/blob/main/Llama%20Class%20-%20Llama%20Game%20Decomposition.md)
[Obstacle Class - Llama Game Decomposition](https://github.com/josephsurrey/Llama-Game/blob/main/Obstacle%20Class%20-%20Llama%20Game%20Decomposition.md)
[Scoreboard Class - Llama Game Decomposition](https://github.com/josephsurrey/Llama-Game/blob/main/Scoreboard%20Class%20-%20Llama%20Game%20Decomposition.md)
...
| Test Case                      | Scenario                                                     | Key Interactions Tested                                                                                                 | Expected Outcome/State Checks                                                                                                                                                                                 | Test Type   |
| :----------------------------- | :----------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---------- |
| **Game Initialization**        | `Game()` is instantiated.                                    | `__init__` execution, Pygame module init, screen setup, clock creation, sprite group setup, Llama/Scoreboard creation.  | Game object created, `running`=True, `game_over`=False, `llama` instance present, `scoreboard` instance present, `all_sprites` contains llama, `obstacles` empty, `start_time` set, obstacle timer scheduled. | Integration |
| **Basic Gameplay Loop (Tick)** | Simulate one frame update during gameplay.                   | `_update` -> `all_sprites.update`, `scoreboard.update`, `_check_collisions`.                                            | `llama.update` called, `scoreboard.update` called (score might increase), `_check_collisions` called, `game_over` remains False.                                                                              | Integration |
| **Player Jump Event**          | Simulate SPACE key press during gameplay.                    | `_handle_events` -> `llama.jump`.                                                                                       | `llama.jump` method is invoked.                                                                                                                                                                               | Integration |
| **Obstacle Spawn Event**       | Simulate `OBSTACLE_SPAWN_EVENT` during gameplay.             | `_handle_events` -> `_spawn_obstacle` -> `Obstacle` creation, adding to groups.                                         | New `Obstacle` added to `obstacles` and `all_sprites` groups.                                                                                                                                                 | Integration |
| **Collision Detection**        | Simulate llama colliding with an obstacle during gameplay.   | `_update` -> `_check_collisions` -> `pygame.sprite.spritecollide` returns collision.                                    | `game_over` becomes True, obstacle spawn timer is cancelled (`set_timer` with 0).                                                                                                                             | Integration |
| **Game Over State (No Input)** | Game is in the `game_over` state. Simulate one frame update. | `_update` logic bypass, `_draw` logic for game over screen.                                                             | `all_sprites.update`, `scoreboard.update`, `_check_collisions` are *not* called. Drawing methods for "Game Over" text, score, and instructions are called.                                                    | Integration |
| **Restart Event**              | Simulate 'R' key press during game over.                     | `_handle_events` -> `_reset_game`.                                                                                      | `game_over` becomes False, `obstacles` cleared, `all_sprites` reset (only llama), `llama.reset` called, `scoreboard.reset` called, `start_time` updated, obstacle timer restarted.                            | Integration |
| **Quit Event (Game Over)**     | Simulate 'Q' key press during game over.                     | `_handle_events` -> sets `self.running` to False.                                                                       | `game.running` becomes False.                                                                                                                                                                                 | Integration |
